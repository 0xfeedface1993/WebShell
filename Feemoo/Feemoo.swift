...
extension ViewController {
    func loadFeemooSequenceBullet() {
        ...
        let mainPage = WebBullet(successAction: {
           ...
           injectJavaScript: "\(functionScript) getSecondPageLinkAndFileName();")
        
        let secondPage = WebBullet(successAction: {
            dat in
           ...
           url: URL(string: urlString)!,
           injectJavaScript: "\(functionScript) getCodeImageAndCodeEncry();")
    ...
    func loadFeemooDownloadLink(code: String) {
        self.webview.evaluateJavaScript("com_down('\(fileid)', '\(code)', null);", completionHandler: { (datx, err) in
            if let e = err {
                print(e.localizedDescription)
                return
            }
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2, execute: {
                self.webview.evaluateJavaScript("fetchDownloadLink();", completionHandler: { (daty, erry) in
